use task_task_revolution::domain::financial::{BudgetStatus, CostEntry, CostSummary, CostType, ProjectBudget};
...
fn test_budget_status_calculation() {
    let mut budget = ProjectBudget::new("PROJ-1".to_string(), 10000.0, "USD".to_string(), "user1".to_string());
