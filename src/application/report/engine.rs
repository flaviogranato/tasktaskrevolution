    ...
    CoR: CompanyRepository,
{
    ...
    pub fn export_report(&self, config: &ReportConfig, file_path: &str) -> Result<ReportResult, AppError> {
        ...
        formatter.format_to_file(&data, file_path)
    ...
    fn task_to_json(&self, task: &AnyTask) -> Result<HashMap<String, serde_json::Value>, AppError> {
        ...
        data.insert("description".to_string(), serde_json::Value::String(task.description().unwrap_or_default().to_string()));
        data.insert("project_code".to_string(), serde_json::Value::String(task.project_code().to_string()));
        data.insert("status".to_string(), serde_json::Value::String(task.status().to_string()));
        data.insert("priority".to_string(), serde_json::Value::String("medium".to_string()));
        data.insert("start_date".to_string(), serde_json::Value::String(task.start_date().format("%Y-%m-%d").to_string()));
        data.insert("due_date".to_string(), serde_json::Value::String(task.due_date().format("%Y-%m-%d").to_string()));
        data.insert("assigned_resources".to_string(), serde_json::Value::Number(serde_json::Number::from(task.assigned_resources().len())));
        Ok(data)
    }
    ...
    fn resource_to_json(&self, resource: &AnyResource) -> Result<HashMap<String, serde_json::Value>, AppError> {
        ...
        data.insert("code".to_string(), serde_json::Value::String(resource.code().to_string()));
        data.insert("name".to_string(), serde_json::Value::String(resource.name().to_string()));
        data.insert("email".to_string(), serde_json::Value::String(resource.email().unwrap_or_default().to_string()));
        data.insert("resource_type".to_string(), serde_json::Value::String(resource.resource_type().to_string()));
        data.insert("status".to_string(), serde_json::Value::String(resource.status().to_string()));
        data.insert("capacity".to_string(), serde_json::Value::Number(serde_json::Number::from(100)));
        data.insert("cost_per_hour".to_string(), serde_json::Value::Number(serde_json::Number::from(0)));
        Ok(data)
    }
    ...
    fn project_to_json(&self, project: &AnyProject) -> Result<HashMap<String, serde_json::Value>, AppError> {
        ...
        data.insert("code".to_string(), serde_json::Value::String(project.code().to_string()));
        data.insert("name".to_string(), serde_json::Value::String(project.name().to_string()));
        data.insert("description".to_string(), serde_json::Value::String(project.description().unwrap_or(&String::new()).to_string()));
        data.insert("company_code".to_string(), serde_json::Value::String(project.company_code().to_string()));
        data.insert("status".to_string(), serde_json::Value::String(project.status().to_string()));
        data.insert("priority".to_string(), serde_json::Value::String("medium".to_string()));
        data.insert("start_date".to_string(), serde_json::Value::String(project.start_date().unwrap_or_default().format("%Y-%m-%d").to_string()));
        data.insert("end_date".to_string(), serde_json::Value::String(project.end_date().unwrap_or_default().format("%Y-%m-%d").to_string()));
        data.insert("task_count".to_string(), serde_json::Value::Number(serde_json::Number::from(project.tasks().len())));
        Ok(data)
    }
    ...
    fn company_to_json(&self, company: &Company) -> Result<HashMap<String, serde_json::Value>, AppError> {
        ...
        data.insert("code".to_string(), serde_json::Value::String(company.code().to_string()));
        data.insert("name".to_string(), serde_json::Value::String(company.name().to_string()));
        data.insert("email".to_string(), serde_json::Value::String(company.email.clone().unwrap_or_default()));
        data.insert("status".to_string(), serde_json::Value::String(company.status.to_string()));
        data.insert("created_at".to_string(), serde_json::Value::String(company.created_at.format("%Y-%m-%d %H:%M:%S").to_string()));
        data.insert("updated_at".to_string(), serde_json::Value::String(company.updated_at.format("%Y-%m-%d %H:%M:%S").to_string()));
        Ok(data)
    }

    /// Aplica filtros aos dados
    fn apply_filters(&self, data: Vec<HashMap<String, serde_json::Value>>, filters: &[ReportFilter]) -> Result<Vec<HashMap<String, serde_json::Value>>, AppError> {
    ...
    fn matches_filter(&self, record: &HashMap<String, serde_json::Value>, filter: &ReportFilter) -> bool {
        ...
        match filter.operator {
            FilterOperator::Equal => field_value.as_str().map_or(false, |s| s == filter_value),
            FilterOperator::NotEqual => field_value.as_str().map_or(true, |s| s != filter_value),
    ...
    fn apply_sorting(&self, mut data: Vec<HashMap<String, serde_json::Value>>, sort_by: &Option<String>, sort_order: &Option<SortOrder>) -> Result<Vec<HashMap<String, serde_json::Value>>, AppError> {
    ...
    fn apply_grouping(&self, data: Vec<HashMap<String, serde_json::Value>>, _group_by: &Option<GroupBy>) -> Result<Vec<HashMap<String, serde_json::Value>>, AppError> {
