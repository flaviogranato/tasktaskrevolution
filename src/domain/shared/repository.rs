...
pub trait TransactionalRepository<T, ID>: Repository<T, ID> {
    /// Begin a transaction
    fn begin_transaction(&self) -> DomainResult<Box<dyn Transaction>>;

    /// Execute a function within a transaction
    fn with_transaction<F, R>(&self, f: F) -> DomainResult<R>
    where
        F: FnOnce(&dyn Transaction) -> DomainResult<R>;
}
...
pub trait Transaction {
    ...
    fn rollback(self: Box<Self>) -> DomainResult<()>;
}
...
where
    T: Clone,
{
    fn find_by_id(&self, id: &String) -> DomainResult<Option<T>> {
    ...
    fn delete(&self, id: &String) -> DomainResult<bool> {
        let mut entities = self.entities_mut();
        let deleted = entities.remove(id).is_some();
        Ok(deleted)
    }

    fn exists(&self, id: &String) -> DomainResult<bool> {
        let entities = self.entities();
...
mod tests {
    ...
    impl Repository<MockEntity, String> for MockRepository {
        fn find_by_id(&self, id: &String) -> DomainResult<Option<MockEntity>> {
            let entities = self.entities.lock().unwrap();
            Ok(entities.get(id).cloned())
        }

        fn find_all(&self) -> DomainResult<Vec<MockEntity>> {
            let entities = self.entities.lock().unwrap();
            Ok(entities.values().cloned().collect())
        }

        fn save(&self, entity: MockEntity) -> DomainResult<MockEntity> {
            let mut entities = self.entities.lock().unwrap();
            entities.insert(entity.id.clone(), entity.clone());
            Ok(entity)
        }

        fn update(&self, entity: MockEntity) -> DomainResult<MockEntity> {
            let mut entities = self.entities.lock().unwrap();
    ...
    impl PaginatedRepository<MockEntity, String> for MockPaginatedRepository {
        fn find_with_pagination(&self, page: usize, size: usize) -> DomainResult<PaginatedResult<MockEntity>> {
    ...
    impl SearchableRepository<MockEntity, String> for MockSearchableRepository {
        fn search(&self, criteria: &SearchCriteria) -> DomainResult<Vec<MockEntity>> {
            let all_entities = self.find_all()?;
        ...
        fn search_with_pagination(
            ...
            page: usize,
            size: usize,
        ) -> DomainResult<PaginatedResult<MockEntity>> {
    ...
    impl CachedRepository<MockEntity, String> for MockCachedRepository {
        ...
        fn invalidate_cache(&self, id: &String) -> DomainResult<()> {
            let cache_key = self.cache_key(id);
            let mut cache = self.cache.lock().unwrap();
