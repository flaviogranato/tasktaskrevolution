...
mod tests {
    ...
    fn test_task_error_display_formatting() {
        ...
        assert_eq!(
            format!("{invalid_date_err}"),
            "Validation error in field 'date_range': Invalid date range"
        );

        let vacation_err =
            DomainError::business_rule_violation("resource_availability", "Resource RES-123 is on vacation");
        assert_eq!(
            format!("{vacation_err}"),
