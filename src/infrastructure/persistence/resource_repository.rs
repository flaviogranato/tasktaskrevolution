...
mod tests {
    ...
    fn test_check_if_layoff_period() {
        ...
        use crate::domain::project::model::ProjectStatus;
        ...
        let layoff_period = LayoffPeriod::new("2024-01-01".to_string(), "2024-01-31".to_string());
        let vacation_rules =
            VacationRules::new(Some(1), Some(true), Some(false), Some(vec![layoff_period]));
        ...
        let start_date = chrono::NaiveDate::from_ymd_opt(2024, 1, 1)
            ...
            .and_hms_opt(0, 0, 0)
            .unwrap();
        let end_date = chrono::NaiveDate::from_ymd_opt(2024, 1, 31)
            .unwrap()
            .and_hms_opt(0, 0, 0)
            .unwrap();
        let offset = Local::now().offset().fix();
        ...
        let start_date2 = chrono::NaiveDate::from_ymd_opt(2024, 2, 1)
            .unwrap()
            .and_hms_opt(0, 0, 0)
            .unwrap();
        let end_date2 = chrono::NaiveDate::from_ymd_opt(2024, 2, 28)
            .unwrap()
            .and_hms_opt(0, 0, 0)
            .unwrap();
