...
use crate::application::financial::{
    AddCostUseCase, CalculateCostsUseCase, ManageBudgetUseCase,
    GenerateReportUseCase, ReportFormat,
};
...
impl CostCommand {
    pub fn execute(&self, base_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        match self {
            CostCommand::Add { resource, amount, cost_type, task, project, description } => {
                self.handle_add_cost(base_path, resource, *amount, cost_type, task, project, description)
            }
            CostCommand::Calculate { project } => {
                self.handle_calculate_costs(base_path, project)
            }
            CostCommand::Budget { command } => {
                self.handle_budget_command(base_path, command)
            }
            CostCommand::Report { project, format } => {
                self.handle_generate_report(base_path, project, format)
            }
        }
    }
    ...
    fn handle_budget_command(
        ...
        match command {
            BudgetCommand::Set { project, amount, currency } => {
                let budget = use_case.set_budget(
                    project.clone(),
                    *amount,
                    currency.clone(),
                    "system".to_string(),
            ...
            BudgetCommand::Status { project } => {
                match use_case.get_budget(project) {
                    Ok(budget) => {
