    ...
    match command {
        ...
        CreateCommand::Project {
            ...
            match use_case.execute(&name, description.as_deref(), company_code.clone(), code) {
                Ok(project) => {
                    println!(
                        "Project '{}' created successfully with code '{}' in company '{}'",
                        project.name(),
                        project.code(),
                        company_code
                    );
        ...
        CreateCommand::Task {
            ...
            // Parse dates
            let start_date_parsed =
                NaiveDate::parse_from_str(&start_date, "%Y-%m-%d").map_err(|e| AppError::ValidationError {
                    field: "start_date".to_string(),
                    message: format!("Invalid start date format: {}", e),
                })?;

            let due_date_parsed =
                NaiveDate::parse_from_str(&due_date, "%Y-%m-%d").map_err(|e| AppError::ValidationError {
